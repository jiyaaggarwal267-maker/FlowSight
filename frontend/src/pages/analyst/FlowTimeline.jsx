...
function FlowTimeline() {
  ...
  const nodeAccounts = accounts.slice(0, 5)
  const nodePos = [
    { x: 285, y: 220 },
    { x: 670, y: 210 },
    { x: 470, y: 360 },
  ...
  return (
    <div className="flex flex-col w-full pb-space-2xl">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-space-md py-space-md mb-space-base">
        ...
        <div className="flex items-center gap-space-sm self-start lg:self-center flex-wrap">
          <div className="flex items-center gap-space-2xs bg-surface-container-high px-space-sm py-1.5 rounded-full text-on-surface-variant font-label-caps text-label-caps">
            ...
            <span>{inv ? `LIVE REPLAY STAGE · DAY ${Math.max(days.length, 0)}` : "LOADING CASE…"}</span>
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-12 gap-space-lg items-start">
        {/* Visual Graph Stage & Inline Alerts (9 Cols) */}
        <div className="xl:col-span-9 flex flex-col gap-space-md">
          {/* Graph Canvas Container */}
          <div className="relative w-full h-[580px] bg-surface-container-lowest rounded shadow-sm overflow-hidden flex flex-col justify-between">
            ...
            <div className="relative z-20 flex items-center justify-between p-space-md">
              <div className="flex items-center gap-space-xs bg-surface/90 backdrop-blur-md px-space-sm py-1 rounded shadow-sm">
                <span className="w-2.5 h-2.5 rounded-full bg-error"></span>
                <span className="font-label-caps text-label-caps text-on-surface uppercase tracking-wider">Topology Snapshot: Network Active</span>
                <span className="text-outline">·</span>
                <span className="font-label-sm text-label-sm text-on-surface-variant">{timeline?.total_txns || 0} Txns Over {spanDays} Days</span>
              </div>
            </div>
            {/* Forensic Interactive Visual Network Diagram (SVG Visual Layer) */}
            <div className="absolute inset-0 z-10 flex items-center justify-center">
              <svg className="w-full h-full" fill="none" viewBox="0 0 960 540" xmlns="http://www.w3.org/2000/svg">
                <defs>
                  <marker id="arrow-crimson" markerHeight="6" markerWidth="6" orient="auto-start-reverse" refX="6" refY="5" viewBox="0 0 10 10">
                    <path d="M 0 1.5 L 8 5 L 0 8.5 z" fill="#ba1a1a"></path>
                  </marker>
                  <marker id="arrow-secondary" markerHeight="6" markerWidth="6" orient="auto-start-reverse" refX="6" refY="5" viewBox="0 0 10 10">
                    <path d="M 0 2 L 6 5 L 0 8 z" fill="#747686"></path>
                  </marker>
                ...
                {/* Edges between real scoped accounts */}
                {nodeAccounts.length > 1 && nodeAccounts.slice(1).map((_, i) => {
                  const a = nodePos[0]
                  const b = nodePos[i + 1] || nodePos[(i % 4) + 1]
                  return (
                    <path key={`e-${i}`} d={`M ${a.x} ${a.y} C ${a.x + 60} ${a.y - 60}, ${b.x - 60} ${b.y - 60}, ${b.x} ${b.y}`} fill="none" markerEnd="url(#arrow-crimson)" stroke="#ba1a1a" strokeDasharray="6 3" strokeWidth="2.5"></path>
                  )
                })}
                ...
                {nodeAccounts.map((acct, idx) => {
                  ...
                  return (
                    <g key={acctId} className="cursor-pointer group">
                      ...
                      <circle cx={p.x} cy={p.y} r={isHub ? 28 : 20} fill="#ffffff" stroke={isHub ? "#ba1a1a" : "#747686"} strokeWidth={isHub ? 3 : 1.5}></circle>
                      <text fill={isHub ? "#ba1a1a" : "#191c1e"} fontFamily="Inter" fontSize={isHub ? 12 : 10} fontWeight="700" textAnchor="middle" x={p.x} y={p.y + 3}>{acctId}</text>
                      <text fill="#747686" fontFamily="Inter" fontSize="9" fontWeight="600" textAnchor="middle" x={p.x} y={p.y + 18}>{name}</text>
                      {isHub && <circle cx={p.x + 22} cy={p.y - 20} r="8" fill="#ba1a1a"></circle>}
                    </g>
                  )
                })}
              </svg>
            </div>
            {/* Inline Forensics Alert Overlay Card */}
            <div className="relative z-30 m-space-md max-w-xl self-start bg-surface-container-lowest/95 backdrop-blur-md rounded p-space-base shadow-lg">
              <div className="flex items-start justify-between gap-space-md mb-space-sm">
                <div className="flex items-center gap-space-xs">
                  <span className="p-1 rounded bg-error-container text-error flex items-center justify-center">
                    <MaterialIcon name="cyclone" className="text-[20px]" />
                  </span>
                  <div className="flex flex-col">
                    <span className="font-label-caps text-label-caps text-error font-semibold uppercase tracking-wider">Pattern Detected</span>
                    <span className="font-headline-sm text-headline-sm text-on-surface">{patternLabel(pattern)}</span>
                  </div>
                </div>
                <span className="px-space-xs py-0.5 rounded bg-surface-container text-outline font-label-caps text-label-caps">{inv?.status ? inv.status.toUpperCase() : "N/A"}</span>
              </div>
              <div className="grid grid-cols-4 gap-space-xs bg-surface-container-low p-space-sm rounded mb-space-md text-center">
                <div className="flex flex-col">
                  <span className="font-label-caps text-label-caps text-outline uppercase">Accounts</span>
                  <span className="font-numeric-md text-numeric-md text-on-surface font-semibold">{accounts.length} Linked</span>
                </div>
                <div className="flex flex-col">
                ...
                <div className="flex flex-col">
                  ...
                  <span className="font-numeric-md text-numeric-md text-on-surface font-semibold">{formatINR(totalVolume)}</span>
                </div>
                <div className="flex flex-col">
                  <span className="font-label-caps text-label-caps text-outline uppercase">Risk Score</span>
                  <span className="font-numeric-md text-numeric-md text-error font-semibold">{risk} / 100</span>
                </div>
              </div>
              <p className="font-body-sm text-body-sm text-on-surface-variant leading-relaxed">{summary.evidence || summary.description || "No evidence summary available."}</p>
              <div className="flex items-center justify-between gap-space-sm mt-space-sm">
                <span className="font-body-sm text-body-sm text-on-surface-variant flex items-center gap-1">
                  <MaterialIcon name="verified" className="text-[16px] text-primary" />
                  Case {id} · {patternLabel(pattern)}
                </span>
                <Link className="px-space-md py-1 rounded bg-primary text-on-primary hover:bg-primary-container font-label-sm text-label-sm transition-colors shadow-sm" to={`/analyst/investigations/${id}`}>
                  Open Investigation {id}
                </Link>
              </div>
            </div>
            {/* Legend Ribbon in Graph Footer */}
            <div className="relative z-20 px-space-md py-space-xs bg-surface-container-low/90 backdrop-blur-sm flex items-center justify-between font-label-caps text-label-caps text-on-surface-variant">
              <div className="flex items-center gap-space-md">
                <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-error"></span> Primary Account</span>
                <span className="flex items-center gap-1.5"><span className="w-2.5 h-2.5 rounded-full bg-outline"></span> Scoped Account</span>
              </div>
              <div className="flex items-center gap-space-xs text-outline">
                <span>{accounts.length} accounts in scope for {id}</span>
              </div>
            </div>
          </div>

          {/* Playback Timeline & Scrubber Controller Bar */}
          <div className="w-full bg-surface-container-lowest rounded shadow-sm p-space-base flex flex-col gap-space-md">
            ...
            <div className="relative py-space-sm px-space-xs">
              <div className="w-full h-2 rounded-full bg-surface-container-high relative overflow-hidden">
                <div className="h-full bg-primary rounded-full transition-all duration-300" style={{ width: days.length ? "100%" : "0%" }}></div>
              </div>
              <div className="relative w-full flex justify-between items-center -mt-6">
                {days.map((d, i) => (
                  <button key={d.day} className="group flex flex-col items-center focus:outline-none cursor-pointer" type="button" onClick={() => notify({ title: `Day ${d.day}`, body: `${d.txn_count} txns · ${formatINR(d.volume)} on ${d.date}.`, tone: "primary" })}>
                    <span className={`rounded-full flex items-center justify-center font-label-caps ${i === days.length - 1 ? "w-7 h-7 bg-error text-on-error font-numeric-md font-bold shadow-md ring-4 ring-error-container" : "w-5 h-5 bg-primary text-on-primary text-[10px] shadow-sm"}`}>{d.day}</span>
                    <span className={`mt-2 font-label-caps text-label-caps ${i === days.length - 1 ? "text-error font-bold uppercase tracking-wider" : "text-on-surface-variant"}`}>{i === days.length - 1 ? "Active Pin" : `Day ${d.day}`}</span>
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-space-sm pt-space-xs">
              ...
              <div className="flex items-center gap-space-sm">
                <div className="flex items-center gap-1 bg-surface-container-low p-1 rounded">
                  ...
                  <span className="px-2 py-0.5 rounded font-label-sm text-label-sm text-on-surface font-semibold bg-surface-container-lowest shadow-sm">{days.length ? days[days.length - 1].txn_count + " Txn(s)" : "0"}</span>
                </div>
                <button className="flex items-center gap-space-xs px-space-md py-1.5 rounded bg-surface-container-low hover:bg-surface-container-high text-on-surface font-label-sm text-label-sm transition-colors shadow-sm cursor-pointer" type="button" onClick={() => notify({ title: "Frame captured", body: `${id} topology snapshot saved to case vault.`, tone: "primary" })}>
          ...
          <div className="bg-surface-container-lowest rounded shadow-sm p-space-base flex flex-col gap-space-base">
            ...
            <div className="flex flex-col gap-space-sm relative">
              ...
              {days.map((d, i) => {
                ...
                return (
                  <div key={d.day} className="relative pl-8 flex flex-col gap-space-2xs">
                    {isLast ? (
                      <>
                    ...
                    <div className={`p-space-sm rounded flex flex-col gap-1 ${isLast ? "bg-error-container/40 shadow-sm" : "bg-surface-container-low"}`}>
                      <div className="flex items-center justify-between">
                        <span className={`font-label-caps text-label-caps font-semibold ${isLast ? "text-error font-bold uppercase" : "text-primary"}`}>DAY {d.day}{isLast ? " · CURRENT FRAME" : ""}</span>
                        <span className={`font-numeric-md text-numeric-md font-semibold ${isLast ? "text-error" : "text-on-surface"}`}>{d.active_accounts} Accts</span>
                      </div>
            ...
            <div className="mt-space-xs p-space-sm rounded bg-surface-container-low flex flex-col gap-space-xs">
              ...
              <div className="h-14 w-full flex items-end justify-between gap-1 pt-2">
                {days.map((d, i) => {
                  ...
                  return (
                    <div key={d.day} title={`Day ${d.day}: ${formatINR(d.volume)}`} className={`w-full rounded-t ${i === days.length - 1 ? "bg-error" : "bg-primary-container"}`} style={{ height: `${h}%` }}></div>
                  )
                })}
              </div>
