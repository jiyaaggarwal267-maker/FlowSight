...
function LandingGraph({ tall = false }) {
    ...
    >
      <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          <div className="flex flex-wrap items-center gap-1 rounded-lg bg-surface-container-low p-1 border border-outline-variant/60">
            {FILTERS.map((f) => (
              <button
                key={f.key}
                type="button"
                ...
                className={
                  activeFilter === f.key
                    ? "px-2.5 py-1 text-xs font-semibold rounded bg-primary text-white shadow-sm flex items-center gap-1 cursor-pointer"
                    : "px-2.5 py-1 text-xs font-medium rounded hover:bg-surface-container-lowest text-secondary transition-colors cursor-pointer"
                }
              >
                {activeFilter === f.key ? <MaterialIcon name="check" className="text-sm" /> : null}
                {f.label}
              </button>
            ))}
          </div>
          <button
            ...
            className={`px-2.5 py-1 text-xs font-semibold rounded flex items-center gap-1.5 border transition-colors cursor-pointer ${
              loopFocus
                ? "bg-primary border-primary text-white"
                : "bg-surface-container-lowest border-outline-variant/60 text-primary hover:bg-surface-container"
            }`}
          >
            <span className={`w-1.5 h-1.5 rounded-full ${loopFocus ? "bg-white animate-ping" : "bg-primary"}`}></span>
      ...
      >
            ...
            {n.icon ? (
              <>
                <div className="flex items-center gap-1.5">
                  ...
                  <span className={`font-mono text-xs font-semibold ${n.red ? "text-red-400 font-bold" : "text-slate-200 font-semibold"}`}>{n.title}</span>
                </div>
                <p className={`text-xs font-mono mt-0.5 ${n.red ? "text-red-400" : "text-slate-400"}`}>{n.sub}</p>
              </>
            ) : (
              <>
                <div className="flex items-center gap-1.5">
                ...
                <p className="text-xs text-slate-400 font-mono mt-0.5">{n.sub}</p>
              </>
            )}
          </div>
        ))}
        ...
        <div className="hidden md:block absolute left-[340px] top-[24px] z-30 w-72 bg-slate-950/95 border border-red-500/80 rounded-xl shadow-2xl p-3.5 backdrop-blur-md text-white transition-all">
          <div className="flex items-start justify-between pb-2 border-b border-slate-800">
            ...
            <span className="px-1.5 py-0.5 bg-red-500/20 text-red-400 border border-red-500/40 rounded text-xs font-mono font-bold">CRITICAL</span>
          </div>
          <div className="py-2.5 space-y-1.5 font-mono text-xs">
            ...
            <div className="flex justify-between">
              ...
              <span className="text-blue-400">14 Layered Nodes / 3 Banks</span>
            </div>
          </div>
          <div className="pt-2 border-t border-slate-800 flex gap-2">
            <Link to="/analyst/network-explorer" className="flex-1 py-1 bg-primary text-white rounded text-xs font-medium hover:bg-blue-600 transition-colors text-center cursor-pointer">
              Inspect in Explorer
            </Link>
            <button className="py-1 px-2.5 bg-slate-800 text-slate-200 border border-slate-700 rounded text-xs font-medium hover:bg-slate-700 transition-colors cursor-pointer" type="button" onClick={() => navigate("/analyst/reports")}>
              Generate FIU Report
            </button>
          </div>
        </div>
        ...
        <div className="absolute bottom-3 left-3 right-3 flex flex-wrap items-center justify-between gap-y-1 bg-slate-950/80 px-3 py-2 rounded-lg border border-slate-800 text-xs font-mono">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-slate-400">
            ...
            <span className="flex items-center gap-1.5">
              ...
              <span className="text-slate-200">Suspicious Relay (50-89)</span>
            </span>
            <span className="flex items-center gap-1.5">
              <span className="w-2.5 h-2.5 rounded-full bg-emerald-500"></span>
              <span className="text-slate-200">Legitimate Origin (&lt;20)</span>
            </span>
          </div>
      ...
      <div className="md:hidden flex flex-col gap-3">
        <div className="rounded-xl border border-red-500/80 bg-slate-950/95 p-3.5 text-white">
          ...
          <div className="py-2.5 space-y-1.5 font-mono text-xs">
            ...
            <div className="flex justify-between gap-2">
              <span className="text-slate-400">Connected Clusters:</span>
              <span className="text-blue-400 text-right">14 Layered / 3 Banks</span>
            </div>
          </div>
          <div className="pt-2 border-t border-slate-800 flex flex-col gap-2">
            <Link to="/analyst/network-explorer" className="flex-1 py-1.5 bg-primary text-white rounded text-xs font-medium hover:bg-blue-600 transition-colors text-center cursor-pointer">
              Inspect in Explorer
            </Link>
            <button className="py-1.5 px-2.5 bg-slate-800 text-slate-200 border border-slate-700 rounded text-xs font-medium hover:bg-slate-700 transition-colors cursor-pointer" type="button" onClick={() => navigate("/analyst/reports")}>
              Generate FIU Report
            </button>
          </div>
        </div>
              ...
              {n.icon ? (
                <>
                  <div className="flex items-center gap-1.5 min-w-0">
                    ...
                    <span className={`font-mono text-xs truncate ${n.red ? "text-red-400 font-bold" : "text-slate-200 font-semibold"}`}>{n.title}</span>
                  </div>
                  <p className={`text-xs font-mono mt-0.5 truncate ${n.red ? "text-red-400" : "text-slate-400"}`}>{n.sub}</p>
                </>
              ) : (
                <>
                  <div className="flex items-center gap-1.5 min-w-0">
                    <span className={`w-2 h-2 rounded-full shrink-0 ${n.dot}`}></span>
                    <span className="font-mono text-xs truncate text-white font-semibold">{n.title}</span>
                  </div>
                  <p className="text-xs text-slate-400 font-mono mt-0.5 truncate">{n.sub}</p>
                </>
              )}
            </div>
          ))}
